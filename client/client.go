...
func parseServerConfig(config *ss.Config) {
	...
	if len(config.ServerPassword) == 0 {
		...
		for _, serverInfo := range config.ServerPassword {
			...
			if !ok {
				cipher, _ = ss.NewCipher(encmethod, passwd)
	...
	// log.Println("listen socks5 server at", se.server)
	// }
	return
}
...
func handleConnection(conn net.Conn) {
	...
	if err != nil {
		if len(servers.srvCipher) > 1 {
			// log.Println("Failed connect to all available server")
		}
		return
	}
	defer func() {
...
func run(listenAddr string) {
	...
	log.Printf("start http server at 9090")

	// HttpGetFromProxy()
	go http.ListenAndServe("127.0.0.1:9090", http.HandlerFunc(serveHTTP))
	for {
		conn, err := ln.Accept()
		if err != nil {
...
func serveHTTP(w http.ResponseWriter, req *http.Request) {
	...
	dialer, _ := proxy.SOCKS5("tcp", "127.0.0.1:9092", nil, d)
...
func handleHTTP(w http.ResponseWriter, req *http.Request, dialer proxy.Dialer) {
	// log.Printf("handleHTTP")
	tp := http.Transport{
		Dial: dialer.Dial,
	}
	resp, err := tp.RoundTrip(req)
	if err != nil {
		// log.Printf("handleHTTP error")
	...
	io.Copy(w, resp.Body)
	// log.Printf("handleHTTP end status:", resp.StatusCode)
}
...
func main() {
	...
	cmdConfig.LocalAddress = "127.0.0.1"
	...
	cmdConfig.LocalPort = 9092
	// flag.StringVar(&cmdConfig.Method, "m", "", "encryption method, default: aes-256-cfb")
	cmdConfig.Method = "aes-128-cfb"
